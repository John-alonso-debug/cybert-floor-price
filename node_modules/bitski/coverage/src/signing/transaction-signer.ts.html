...
<a name='L161'></a><a href='#L161'>161</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">7x</span>
...
&nbsp;
  public async sign(transaction: Transaction, accessToken: string): Promise&lt;string&gt; {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.callbackURL) {
      const persisted = <span class="cstat-no" title="statement not covered" >await this.submitTransaction(transaction, accessToken);</span>
<span class="cstat-no" title="statement not covered" >      return this.redirectToCallbackURL(persisted.transaction);</span>
    }
&nbsp;
    this.submitTransaction(transaction, accessToken).catch(<span class="fstat-no" title="function not covered" >(e</span>rror) =&gt; {
<span class="cstat-no" title="statement not covered" >      return this.handleCallback({error});</span>
    });
&nbsp;
...
&nbsp;
  protected <span class="fstat-no" title="function not covered" >redirectToCallbackURL(</span>transaction: Transaction): Promise&lt;string&gt; {
    const url = <span class="cstat-no" title="statement not covered" >`${this.webBaseUrl}/transactions/${transaction.id}?redirectURI=${this.callbackURL}`;</span>
<span class="cstat-no" title="statement not covered" >    window.location.href = url;</span>
